...
- feature: **Task-context export.** Generate a deterministic, scope-limited
  code-intelligence packet for a set of paths — a specy-road task's
  `touch_zones` — from the Smells view. The packet covers the files in scope,
  their heuristic findings, complexity hotspots, the call-graph neighbourhood,
  and the boundary calls crossing the scope edge (what depends on the scoped
  code, and what it reaches outside), so an agent can review or refactor the
  task without reading the whole repo. Choose **Markdown** (drops straight into
...
- feature: **Graph tab: orientation-first navigation for large repos.** Several changes make hundreds of files navigable instead of an endless sprawl. (1) **Wide layers now wrap, and boxes never overlap.** Previously each import-depth layer was a single horizontal row sized `widest × 200px`, so a layer of 80 files became a ~16,000px-wide strip you had to pan across; layers now wrap into balanced sub-rows (column budget scales with `√(node count)`, capped at 26), keeping the whole map near the viewport's aspect ratio. Nodes within a row are packed by their *measured* width — a folder box is as wide as its longest label, a file as wide as its name — rather than a fixed column pitch, so wide folder boxes no longer collide with their neighbours. Labels also carry a thinner background halo so names read as crisp text instead of fuzzy/bold smudges. Folder boxes are now a neutral violet (with a `folder` legend swatch) so containers no longer blend in with blue-tinted source files, which keep their per-language colors; the selected node fills with a single bright highlight color reserved for the selection, making it unmistakable in a crowded map. (2) **Minimap.** A live thumbnail of the entire graph sits bottom-right of the stage with a draggable viewport rectangle — click or drag it to jump anywhere, so you stay oriented while zoomed in. It appears once a repo has more than 12 nodes and stays in sync with pan/zoom. (3) **Fit-to-content on load.** The graph now opens framed and centered (with a margin) instead of as a microscopic dot-field inside the oversized canvas, and a faint dot-grid backdrop gives the eye fixed spatial anchors while panning. (4) **Edge-to-edge fullscreen.** A `⛶ Fullscreen` button in the graph controls promotes the graph overlay to true browser fullscreen (hiding the app/browser chrome) via the Fullscreen API; `Esc` exits. It targets the persistent overlay element, so clicking nodes doesn't drop you out.
- feature: **Copy a single finding.** Each finding row in the Smells view gets a small `⧉ copy` button (between `open source` and `dismiss`) that copies just that one finding to the clipboard as a compact markdown brief — `file:line`, kind/subkind, severity, function, why-flagged, the heuristic's meaning, and the snippet — ready to paste straight into an LLM. Complements the bulk `Copy prompt` export bar when you only want to hand off one item.
- fix(smells): root-absolute asset URLs in HTML/CSS (e.g. `<link href="/static/app.css">`) are no longer false-flagged as broken relative imports. A leading-slash path is a URL rooted at the web/repo root, not the importing file's directory — the resolver now strips the leading slash and resolves it against the repo root, so `/static/app.css` correctly matches `static/app.css`. When a root-absolute path still doesn't resolve, it's reported as `subkind: 'absolute'` at `info` severity (not `warn`) with a note that a framework static mount may remap the prefix, since Codemap can't model runtime mounts like `app.mount("/static", StaticFiles(directory="static"))`.

## v3.6.0 — 2026-05-28

- fix(smells): the `unresolved-call` detector no longer false-flags Python interpreter builtins (`exit`, `quit`, `breakpoint`, `eval`, `exec`, `__import__`, and other common names) or dunder method calls. Bare dunder calls like `__init__()` / `__post_init__()` are part of the object protocol and are now skipped (any `__name__` call in a Python file), and the missing builtins were added to the `py` `BUILTINS` set.
- feature: **Jump to source from findings.** Clicking a finding's `file:line` location — in the Risk tab and the Smells overlay — now switches the workspace to the **Source** view and scrolls to that line, instead of opening the file in Summary mode at the top. Implemented via a transient `STATE.sourceScrollLine`: the source view tags the matching line row with `data-scroll-target`, and `renderWorkspaceShell` scrolls it into view (`scrollIntoView({ block: 'center' })`) synchronously right after the tree is attached, then clears the target — so layout is available and no later re-render can detach the row first.
- feature: **Dismiss smells.** Each finding row in the Smells overlay gets a small `✕ dismiss` button; dismissals persist in `localStorage` keyed by repo identity (git URL for git loads, file-tree fingerprint for local folders) so they survive reloads. The overlay header shows `N findings · M dismissed` and exposes a `show/hide dismissed` toggle that reveals dismissed rows at reduced opacity with a `↶ restore` button. IDs are derived from `file|kind|subkind|snippet[0:80]` (deliberately excluding `line`) so editing code above a finding does not un-dismiss it.
- feature: **Export / Import triage as JSON.** The smell export bar gets `Export triage` (downloads `codemap-triage-<slug>.json` with the current dismissed set, repo id, version, timestamp) and `Import triage` (file picker, union-merges into the current dismissed set; warns with a `confirm()` dialog if the file's `repoId` differs from the current repo). Lets you share triage decisions across machines without a server.
- fix(smells): the `unresolved-call` detector now recognises every common Python binding site, not just `def`/`class`/`import` and bare-name assignment. Per CLAUDE.md the parser stays regex-only (no AST), so this is implemented by extending the Python `locals` regex set in `lang-config.js` to cover: tuple/star unpacking on assignment (`a, b = …`, `*rest, last = …`), for-loop and comprehension targets (`for x in …`, `[x for x in …]`, including tuple targets `for k, v in …`), `with … as x` and `except … as x` (one capture per `as` clause), and the walrus operator (`x := expr`). `extractLocals` also strips wrapping `*` / `**` / `(` / `[` / `]` so unpacked tokens reduce to bare identifiers. In `smells.js`, parameter names are now normalised by stripping `*` / `**` prefixes so `def f(*args)` correctly binds `args`, not `*args`.
- fix(toolbar): Clear button now always renders (was previously gated behind "has loaded state" and could be missed if the toolbar didn't re-render after load). When nothing is loaded, the button silently clears the URL hash so the next reload starts pristine; when a project is loaded it confirms first. Also moved before the Drop button so it isn't pushed off-screen on narrow toolbars.
- feature: **Clear button in the toolbar.** Since the URL hash now persists the loaded repo (`repo=`) and current view (`file=`/`fn=`/`overlay=`), reloading the page re-hydrates the same project, and there was no in-app way to escape that state. Added a `Clear` button next to `Drop repo` that confirms with the user, then calls `resetAll()` (wipes `STATE.files`, docs, lineage, repo meta, history, selection, filters) and `clearHash()` (removes the URL fragment via `history.replaceState`), returning the app to the pristine empty-drop screen. Button is only rendered when something is loaded.
- fix(smells): the `unresolved-call` detector now recognises three more Python binding mechanisms that previously produced false-positive "hallucinated" findings. (1) Local variables assigned inside a function body — e.g. `handler = dispatch.get(action)` followed by `handler()` — count as defined for later uses in the same scope; the detector now consults `fn.params` and `fn.locals` per call site, not just imports and module-level defs. (2) Module-level bare assignments (`_name = other_name`) at column 0 add `_name` to the file's binding index. (3) Conditional/guarded reassignment of module globals — any module-scope assignment on any control-flow path (e.g. `try: from c import impl / except ImportError: impl = None`) now defines the name. JS gets the same module-level-assignment treatment.
