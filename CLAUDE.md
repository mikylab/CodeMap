...
2. **Update docs** affected by the change — `README.md` for user-facing
   ...
   philosophy changes.
3. **Bump the version** if the change is user-visible. Codemap follows
   [SemVer](https://semver.org): PATCH for bugfixes, MINOR for additive
   features, MAJOR for breaking UI/API/data-shape changes. The version lives in
   two places that must stay in sync:
   - `package.json` → `"version"` field
   - `src/version.js` → `export const VERSION`
   Pure refactors, doc-only edits, and test-only changes do **not** bump the
   version.
4. **Cut a release section in `CHANGELOG.md`** when the user signals a
   release. Rename `## Unreleased` to `## vX.Y.Z — YYYY-MM-DD` and open a
   fresh empty `## Unreleased` above it.
5. **Write a commit message** describing the change. Use the existing style
   (`area: short summary`, e.g. `ui:`, `fix(effects):`, `docs:`).
   **Never** mention Claude, Anthropic, AI, LLMs, "generated by", or
   "Co-Authored-By" trailers — write it as if authored solely by the human
   committer. This rule is absolute, including when the user asks for a
   detailed or multi-paragraph message.
