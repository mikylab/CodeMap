...
function hotspotRow({ file, stats, risk }) {
  ...
  const meta = `cx ${file.cx.toFixed(1)} · ${stats.commits} commit${stats.commits === 1 ? '' : 's'} · ${formatAge(stats.lastTouched)}`;
...
function formatAge(lastTouchedSec) {
  if (!lastTouchedSec) return 'unknown';
  const days = Math.max(0, Math.floor((Date.now() - lastTouchedSec * 1000) / 86_400_000));
