...
test('parser: body extraction spans blank lines (python)', () => {
  const src = `def execute_tool(tool, arguments):\n    """Doc.\n\n    More doc with blanks.\n    """\n    if x == "list":\n        return handle_list(arguments)\n\n    elif x == "count":\n        return handle_count(arguments)\n\n    else:\n        return handle_other(arguments)\n\ndef sibling():\n    pass\n`;
  const out = parseFile('h.py', src, 'h.py');
  const fn = out.fns.find(f => f.name === 'execute_tool');
  assertTrue(fn.calls.includes('handle_list'));
  assertTrue(fn.calls.includes('handle_count'));
  assertTrue(fn.calls.includes('handle_other'));
});

test('parser: body extraction stops at sibling def (python)', () => {
  const src = `def alpha():\n    foo()\n\ndef beta():\n    bar()\n`;
  const out = parseFile('h.py', src, 'h.py');
  const a = out.fns.find(f => f.name === 'alpha');
  assertTrue(a.calls.includes('foo'));
  assertFalse(a.calls.includes('bar'));
});

test('parser: body extraction spans blank lines (js)', () => {
...
test('python: extracts docstring as fn.doc', () => {
  const src = `def greet(name):\n    """Say hi to NAME."""\n    return f"hi {name}"\n`;
  const out = parseFile('a.py', src, 'a.py');
  const fn = out.fns.find(f => f.name === 'greet');
  assertEqual(fn.doc.trim(), 'Say hi to NAME.');
});

test('python: missing docstring -> fn.doc is null', () => {
  const src = `def bare():\n    return 1\n`;
  const out = parseFile('a.py', src, 'a.py');
  const fn = out.fns.find(f => f.name === 'bare');
  assertEqual(fn.doc, null);
});

test('js: jsdoc above function captured', () => {
  const src = `/**\n * Adds two numbers.\n * @param a number\n */\nfunction add(a, b) { return a + b; }\n`;
  const out = parseFile('a.js', src, 'a.js');
  const fn = out.fns.find(f => f.name === 'add');
  assertTrue(fn.doc.includes('Adds two numbers.'));
});

test('js: no jsdoc -> fn.doc null', () => {
  const src = `function bare() { return 1; }\n`;
  const out = parseFile('a.js', src, 'a.js');
  assertEqual(out.fns[0].doc, null);
});

test('go: leading // comment block captured', () => {
  const src = `// Greet says hello.\n// Use carefully.\nfunc Greet() {}\n`;
  const out = parseFile('m.go', src, 'm.go');
  assertTrue(out.fns[0].doc.includes('Greet says hello.'));
});

test('rust: /// triple-slash block captured', () => {
  const src = `/// Adds two numbers\nfn add(a: i32, b: i32) -> i32 { a + b }\n`;
  const out = parseFile('m.rs', src, 'm.rs');
  assertTrue(out.fns[0].doc.includes('Adds two numbers'));
});

test('ruby: # comment block captured', () => {
  const src = `# Greets the user.\ndef greet; end\n`;
  const out = parseFile('a.rb', src, 'a.rb');
  assertTrue(out.fns[0].doc.includes('Greets the user.'));
});

test('java: javadoc captured', () => {
  const src = `/**\n * Computes the answer.\n */\npublic int answer() { return 42; }\n`;
  const out = parseFile('a.java', src, 'a.java');
  assertTrue(out.fns[0].doc.includes('Computes the answer.'));
});

test('python: file-level docstring -> fileDoc', () => {
  const src = `"""Top-level module doc."""\nimport os\ndef x(): pass\n`;
  const out = parseFile('m.py', src, 'm.py');
  assertTrue(out.fileDoc.includes('Top-level module doc'));
});

test('js: top-of-file jsdoc -> fileDoc', () => {
  const src = `/**\n * The thing module.\n */\nfunction a(){}\n`;
  const out = parseFile('m.js', src, 'm.js');
  assertTrue(out.fileDoc.includes('The thing module.'));
});

test('parser: file with no doc -> fileDoc null', () => {
  const src = `function bare(){}\n`;
  const out = parseFile('a.js', src, 'a.js');
  assertEqual(out.fileDoc, null);
});

test('python: extracts params with defaults', () => {
  const src = `def parse(data, opts=None, *, strict=True):\n    pass\n`;
  const fn = parseFile('a.py', src, 'a.py').fns[0];
  assertDeepEqual(fn.params.map(p => p.name), ['data', 'opts', 'strict']);
  assertEqual(fn.params[1].default, 'None');
});

test('js: extracts function params', () => {
  const src = `function add(a, b = 1, ...rest) { return a + b; }\n`;
  const fn = parseFile('a.js', src, 'a.js').fns[0];
  assertDeepEqual(fn.params.map(p => p.name), ['a', 'b', '...rest']);
});

test('python: complex type annotations survive comma split', () => {
  const src = `def fn(x: Dict[str, int], y=(1, 2)) -> int:\n    return 0\n`;
  const fn = parseFile('a.py', src, 'a.py').fns[0];
  assertEqual(fn.params.length, 2);
  assertEqual(fn.params[0].name, 'x');
  assertEqual(fn.params[1].name, 'y');
});

test('parser: zero-arg fn has empty params', () => {
  const src = `def bare():\n    return 1\n`;
  const fn = parseFile('a.py', src, 'a.py').fns[0];
  assertDeepEqual(fn.params, []);
});

test('parser: class line has empty params', () => {
